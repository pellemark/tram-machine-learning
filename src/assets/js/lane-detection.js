...
function toDenormalized(normalized) {
    console.log('toDenormalized');
    return normalized.map(value => value * 255);
}

function toGradientMagnitude(xDerived, yDerived, width, height, ut = 0, lt = 0) {
    console.log('toGradientMagnitude');
...
function startLaneDetection () {
    const toConvolutionForImg = toConvolution(1000, 1000);
    const grayscale = toPixels(toGrayscale(self.imgData, 1000, 1000));
